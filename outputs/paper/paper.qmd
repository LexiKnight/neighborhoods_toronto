...
income_data <- analysis_data %>%
  filter(`Neighbourhood Name` %in% income_categories) %>%
  select(`Neighbourhood Name`, `Rosedale-Moore Park`, `Downtown Yonge East`)

# Prepare the education data
education_data <- analysis_data %>%
  filter(`Neighbourhood Name` %in% education_categories) %>%
  select(`Neighbourhood Name`, `Rosedale-Moore Park`, `Downtown Yonge East`)

# Combine income and education into a new data frame
combined_data <- data.frame(
  Education = rep(education_categories, each = length(income_categories)),  # Repeat education levels
  Income = rep(income_categories, times = length(education_categories))  # Repeat income categories
)

# Ensure the Income column is numeric by mapping the character values to their numeric indices
combined_data$IncomeNumeric <- as.numeric(factor(combined_data$Income, levels = income_categories))

# Set the Education variable as a factor to maintain order
combined_data$Education <- factor(combined_data$Education, levels = education_categories)

# Create the line plot
line_plot <- ggplot(combined_data, aes(x = Education, y = IncomeNumeric)) +
  geom_line(aes(group = 1), color = "blue") +  # Line connecting the points
  geom_point(size = 2, color = "red") +  # Points to mark each category
  scale_y_continuous(breaks = seq_along(income_categories), 
                     labels = income_categories) +  # Set y-axis labels
  labs(
    title = "Line Plot of Income Across Education Levels",
    x = "Education Levels",
    y = "Income Categories"
  ) +
  theme_minimal() +
...
#trying to fix issue of dataset values incorrectly being read?

# Check the income_data
print(income_data)

# Check the education_data
print(education_data)

# Check the combined_data
print(combined_data)

# they all seem to contain the expected values 
